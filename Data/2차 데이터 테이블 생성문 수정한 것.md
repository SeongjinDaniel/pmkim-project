...
/
--위도 경도 삭제
ALTER TABLE shop DROP COLUMN lat;
ALTER TABLE shop DROP COLUMN lng;
desc shop;
...
--COMMENT ON COLUMN shop.lat IS '위도'
/

--COMMENT ON COLUMN shop.lng IS '경도'
/

COMMENT ON COLUMN shop.tel_num IS '전화 번호'
/

COMMENT ON COLUMN shop.shop_name IS '판매점 명'
/
...
COMMENT ON COLUMN board.write_date IS '날짜'
/
...
alter table event modify (event_id int);
...
COMMENT ON COLUMN reply.write_date IS '날짜'
/
...
(
    good_shop_id    INT    NOT NULL, 
    shop_id       VARCHAR2(20)    NOT NULL, 
    good_id         VARCHAR2(20)    NOT NULL, 
    CONSTRAINT GOODS_SHOP_PK PRIMARY KEY (good_shop_id)
)
ALTER TABLE goods_shop RENAME COLUMN shop_name TO shop_id;
ALTER TABLE goods_shop MODIFY (shop_id VARCHAR2(20));
ALTER TABLE goods_shop MODIFY (good_shop_id INT);
desc goods_shop;
/
...
COMMENT ON COLUMN goods_shop.shop_name IS '판매점 명'
/
...
ALTER TABLE goods_shop
    ADD CONSTRAINT FK_goods_shop_shop_name_shop_s FOREIGN KEY (shop_id)
        REFERENCES shop (shop_id)
/
