...
@Service
public class NewsPagingService {
	...
	public int getPageCount() {	// 글의 개수에 따른 페이지 개수 계산
		int postCnt = ndao.allCount();
		//System.out.println("postCnt : "+postCnt);
		return ((postCnt-1)/lineCnt)+1;		
	}
	public int getPageCount(String shop_name_kor) {	// 글의 개수에 따른 페이지 개수 계산
		int postCnt = ndao.sortCount(shop_name_kor);
		//System.out.println("postCnt : "+postCnt);
		return ((postCnt-1)/lineCnt)+1;		
	}
	...
	public int getPageEnd(int pgNum) {	//각 화면별 종료 페이지 번호
		this.pgNum = pgNum;
	...
	public int getPageEnd(int pgNum,String shop_name_kor) {	//각 화면별 종료 페이지 번호
		this.pgNum = pgNum;
		int result1 = getPageStart(pgNum)+pageCnt-1;
		int result2 = getPageCount(shop_name_kor);
		if(result1>result2)
			return result2;
		else
			return result1;
	...
	public boolean isNextData(int pgNum) {	//이전 페이지들을 출력하는 right 이미지 출력 여부 결정
		boolean result = false;
		this.pgNum = pgNum;
		if(getPageEnd(pgNum) < getPageCount())
			result = true;
		return result;
	}
	public boolean isNextData(int pgNum,String shop_name_kor) {	//이전 페이지들을 출력하는 right 이미지 출력 여부 결정
		boolean result = false;
		this.pgNum = pgNum;
		if(getPageEnd(pgNum,shop_name_kor) < getPageCount(shop_name_kor))
			result = true;
		return result;
	}
